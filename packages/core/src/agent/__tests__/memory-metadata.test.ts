...
function memoryMetadataTests(version: 'v1' | 'v2') {
  describe(`${version} - agent memory with metadata`, () => {
    ...
    it('should create a new thread with metadata using generate', async () => {
      ...
      // `agentId` alongside the caller's metadata: a thread row has no agent
      // column, so this is what lets a client list one agent's chats rather
      // than every chat the resource owns.
