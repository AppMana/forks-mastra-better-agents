...
import { getCurrentOrgId, setCurrentOrgId, getToken, loadCredentials } from './chunk-EXG3XKBK.js';
import { randomUUID } from 'crypto';
import fs3, { existsSync, readFileSync, writeFileSync } from 'fs';
import os from 'os';
...
import fs4__default from 'fs/promises';
...
async function globalMCPIsAlreadyInstalled(editor, versionTag) {
  ...
  try {
    ...
    if (editor === 'vscode') {
      ...
      const hasMastraMCP2 = Object.values(configContents.servers).some(server =>
        server?.args?.find(arg => arg === expectedPackage),
      );
      return hasMastraMCP2;
    }
    if (!configContents?.mcpServers) return false;
    const hasMastraMCP = Object.values(configContents.mcpServers).some(server =>
      server?.args?.find(arg => arg === expectedPackage),
    );
    return hasMastraMCP;
  } catch {
...
var DepsService = class {
  ...
  async checkDependencies(dependencies) {
    try {
      ...
      try {
        await fs4__default.access(packageJsonPath);
      } catch {
        return 'No package.json file found in the current directory';
      }
      const packageJson = JSON.parse(await fs4__default.readFile(packageJsonPath, 'utf-8'));
  ...
  async getProjectName() {
    try {
      const packageJsonPath = path.join(process.cwd(), 'package.json');
      const packageJson = await fs4__default.readFile(packageJsonPath, 'utf-8');
  ...
  async addScriptsToPackageJson(scripts) {
    const packageJson = JSON.parse(await fs4__default.readFile('package.json', 'utf-8'));
    packageJson.scripts = {
      ...packageJson.scripts,
      ...scripts,
    };
    await fs4__default.writeFile('package.json', JSON.stringify(packageJson, null, 2));
  }
};
...
var FileEnvService = class _FileEnvService extends EnvService {
  static ENV_KEY_REGEX = /^[A-Za-z_][A-Za-z0-9_]*$/;
  filePath;
  validateEnvEntry(key, value) {
    if (!_FileEnvService.ENV_KEY_REGEX.test(key)) {
  ...
  while (true) {
    ...
    try {
      ...
      if (hadCachedCreds) {
        ...
        if (creds) p2.log.info(`Logged in as ${creds.user.email}`);
      }
      ...
      p2.log.warn(`Could not sign in to Mastra: ${message}`);
    }
  }
}
...
- If the user asks for activities, respond in the format they request.${addExampleTool ? '\n\nUse the weatherTool to fetch current weather data.' : ''}`;
  const imports = [
    ...
    .filter(Boolean)
    .join('\n');
  const toolsConfig = addExampleTool
    ? `  tools: { weatherTool },
`
    : '';
...
`;
  await fs4__default.writeFile(destPath, content);
}
...
var writeIndexFile = async ({ dirPath, addAgent, addExample, addWorkflow, addScorers }) => {
  const indexPath = dirPath + '/index.ts';
  const destPath = path.join(indexPath);
  try {
    await fs4__default.writeFile(destPath, '');
...
var writeObservabilityEnv = async ({ token, projectId, endpoint } = {}) => {
  ...
  lines.push('');
  await fs4__default.appendFile(envFilePath, lines.join('\n'));
};
var createMastraDir = async directory => {
  ...
  try {
    await fs4__default.access(dirPath);
    return { ok: false };
  } catch {
...
var interactivePrompt = async (args = {}) => {
  ...
  if (showBanner) {
    p2.intro(color.inverse(' Mastra Init '));
  }
  const mastraProject = await p2.group(
    {
      ...
      llmApiKey: async ({ results: { llmProvider } }) => {
        ...
        const llmName = LLM_PROVIDERS.find(p3 => p3.value === llmProvider)?.label || 'provider';
...
var checkForPkgJson = async () => {
  ...
  try {
    await fs4__default.access(pkgJsonPath);
  } catch {
    p2.log.error(
...
var readPackageName = async () => {
  try {
    const raw = await fs4__default.readFile(path.join(process.cwd(), 'package.json'), 'utf8');
...
async function writeAgentsMarkdown(options) {
  ...
  await fs4__default.writeFile(filePath, content);
}
async function writeClaudeMarkdown() {
  const filePath = path.join(process.cwd(), 'CLAUDE.md');
  await fs4__default.writeFile(filePath, '@AGENTS.md');
}
...
//# sourceMappingURL=chunk-W6CPGIEC.js.map
//# sourceMappingURL=chunk-W6CPGIEC.js.map
