...
describe('sandboxStatusUrl / sandboxPercent', () => {
  it('addresses the app route and carries the workspace id', () => {
    ...
    expect(sandboxStatusUrl('coding-a b')).toBe(`${appRoute('/workspace/sandbox-status')}?workspaceId=coding-a%20b`);
  });
...
describe('fetchSandboxStatus', () => {
  it('unwraps the status and degrades to null on every failure mode', async () => {
    ...
    expect(await fetchSandboxStatus(undefined, respond({ status: status() }))).toEqual(status());
    expect(await fetchSandboxStatus(undefined, respond({ status: null }))).toBeNull();
    expect(await fetchSandboxStatus(undefined, respond({}))).toBeNull();
    expect(await fetchSandboxStatus(undefined, respond({ status: { phase: 'x' } }))).toBeNull();
    expect(await fetchSandboxStatus(undefined, respond({ status: status() }, false))).toBeNull();

    const thrown = vi.fn(async () => {
      throw new Error('offline');
    }) as unknown as typeof fetch;
    expect(await fetchSandboxStatus(undefined, thrown)).toBeNull();
  });
});
